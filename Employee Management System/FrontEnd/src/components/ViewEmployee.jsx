...
const ViewEmployee = () => {
  ...
  const [phoneNumber,setPhoneNumber] = useState("");  // added hook
  const [salary,setSalary] = useState("");  // added hook
    ...
    const fetchData = async () => {
      ...
      try {
        if (id) {
          ...
          setPhoneNumber(response.data.phoneNumber);// added
          setSalary(response.data.salary);// added
        ...
        <div className="card col-md-6 offser-md-3">
          ...
          <div className="card-body">
            <form>
              ...
              {/* added phone numner */}
              <div className="form-group mb-2 d-flex gap-2">
                <lable className="form-label fw-bold">Phone Number:</lable>
                <div>{phoneNumber}</div>
              </div>

              <div className="form-group mb-2 d-flex gap-2">
                <lable className="form-label fw-bold">Salary:</lable>
                <div>{salary}</div>
              </div>
            </form>
